...
현재 이 저장소는 private이다. GitHub 읽기 권한이 없는 참가자는 설치할 수 없다. 공개 전환은 내부 문서·Git 이력·PR까지 공개하므로 별도 확인이 필요하다. 접근 방식이 확정되기 전에는 참가자 배포 완료로 표시하지 않는다.
...
로컬 카탈로그 설치 검증이며 private 공통 GitHub 저장소에 대한 미인증 참가자 접근 성공을 주장하지 않는다. 원문 수집 활성화·실제 참가자 대화·Windows/macOS 설치 재시험은 이 변경의 검증에 포함하지 않았다.
